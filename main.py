...
from config.path import (aligned_file, best_model_config_file, model_config_file, model_dir, model_file, plot_file,
						 processed_file, vocab_file)
from test import test
from train import train
from utils.dataset import prepare_dataset
from utils.processing import create_vocab, process_dataset
from utils.utils import read_file_to_df, read_json, write_json
...
if __name__ == '__main__':
	...
	if generate:
		...
		vocab_fr, vocab_it = create_vocab(corpus['french'].tolist(), corpus['italian'].tolist())


	corpus_4_model_training, corpus_4_testing = train_test_split(corpus, test_size=0.1)

	config = read_json(model_config_file)
	config['len_vocab_fr'] = vocab_fr.get_max_length()
	config['len_vocab_it'] = vocab_it.get_max_length()
	write_json(config, model_config_file)

	train_dataset, val_dataset, test_dataset = prepare_dataset(corpus_4_model_training, config, vocab_it, vocab_fr)
