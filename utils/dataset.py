...
# 	max_len = 100
# 	padded_tensor = torch.cat((french_sentences, torch.zeros(max_len - french_sentences.size(0))), dim=0)
#
# 	padded_french_sentences = pad_sequence(french_sentences, batch_first=True, padding_value=0)
# 	padded_sequences = F.pad((0, max_len - batch_sequences.size(1)), padding_size, 'constant', value=0)
...
def prepare_dataset(corpus: pd.DataFrame, config: dict):
	...
	train_loader = DataLoader(train_dataset, batch_size=config.get('batch_size'), drop_last=True, shuffle=True)

	val_dataset = LSTMDataset(corpus_fr=val[col[0]], corpus_it=val[col[1]], negative_sampling=False)
	val_loader = DataLoader(val_dataset, batch_size=config.get('batch_size'), drop_last=True, shuffle=True)
